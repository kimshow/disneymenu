...
#### Phase 4: データ収集（部分完了）
- [x] Webスクレイピングスクリプト実装完了 (`scripts/scrape_menus.py` - 180行)
- [x] データ検証スクリプト作成完了 (`scripts/validate_menus.py`)
- [x] テストデータでの検証完了（4件、エラー0件）
- [ ] 実データ収集（ネットワークタイムアウトにより保留中）
  - 公式サイト (https://www.tokyodisneyresort.jp) への接続がタイムアウト
  - 別のネットワーク環境または時間帯での再試行が必要

### ❌ 未着手タスク

#### データ収集（Phase 4 - 継続）
- [ ] 実データ収集の再試行（ネットワーク環境改善後）
  - 対象: 4桁ID 0000-9999
  - 期待取得件数: 200-800件
- [ ] robots.txt 確認（保留中）
