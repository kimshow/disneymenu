...
def validate_menus(file_path: str) -> Dict:
    ...
    results = {"total_count": len(menus), "errors": [], "warnings": [], "statistics": {}}
    ...
    for idx, menu in enumerate(menus):
        for field in required_fields:
            if field not in menu or not menu[field]:
                results["errors"].append(f"Menu {idx} (ID: {menu.get('id', 'unknown')}): Missing or empty '{field}'")
    ...
    if prices:
        ...
        for menu in menus:
            ...
            if price_amount > 10000:
                results["warnings"].append(f"High price detected: {menu['name']} - ¥{price_amount}")
            if price_amount < 100 and price_amount > 0:
                results["warnings"].append(f"Low price detected: {menu['name']} - ¥{price_amount}")
    ...
    results["statistics"][
        "image_coverage"
    ] = f"{menus_with_images}/{len(menus)} ({menus_with_images*100//len(menus) if len(menus) > 0 else 0}%)"
