#!/usr/bin/env python3
"""
メニューに料理種類タグを自動付与するスクリプト

使用方法:
    python scripts/categorize_food_types.py
    python scripts/categorize_food_types.py --dry-run  # 変更をプレビューのみ
"""

import json
import re
from pathlib import Path
from typing import List, Dict, Set
import argparse


# 料理種類のシグネチャ定義（キーワード → タグのマッピング）
FOOD_TYPE_SIGNATURES = {
    "麺類": {
        "keywords": [
            "ラーメン",
            "らーめん",
            "拉麺",
            "うどん",
            "饂飩",
            "そば",
            "蕎麦",
            "パスタ",
            "スパゲッティ",
            "スパゲティ",
            "ペンネ",
            "フェットチーネ",
            "タリオリーニ",
            "リングイネ",
            "ヌードル",
            "フォー",
            "米粉麺",
        ],
        "exclude_keywords": ["ヌードルスープ"],  # 除外キーワード
    },
    "ご飯もの": {
        "keywords": [
            "ライス",
            "ご飯",
            "御飯",
            "白飯",
            "白ご飯",
            "カレー",
            "カレーライス",
            "ピラフ",
            "リゾット",
            "ドリア",
            "チャーハン",
            "炒飯",
            "焼き飯",
            "丼",
            "どんぶり",
            "おにぎり",
            "おむすび",
            "雑炊",
            "リゾット",
        ],
        "exclude_keywords": ["ライスコロッケ", "ライスペーパー"],
    },
    "パン": {
        "keywords": [
            "サンドイッチ",
            "サンド",
            "バーガー",
            "ハンバーガー",
            "ホットドッグ",
            "ホットドック",
            "パン",
            "バンズ",
            "ブレッド",
            "ベーグル",
            "マフィン",
            "トースト",
            "クロワッサン",
            "デニッシュ",
        ],
        "exclude_keywords": ["パンケーキ", "パンナコッタ", "パンプキン", "パンダ", "アクアパンナ"],
    },
    "肉料理": {
        "keywords": [
            "ステーキ",
            "牛肉",
            "ビーフ",
            "ポーク",
            "豚肉",
            "ポークカツ",
            "とんかつ",
            "チキン",
            "鶏肉",
            "チキンレッグ",
            "ローストチキン",
            "ソーセージ",
            "ウインナー",
            "ハム",
            "生ハム",
            "ベーコン",
            "ターキー",
            "七面鳥",
            "ラム",
            "羊肉",
            "ミートボール",
            "ハンバーグ",
        ],
        "exclude_keywords": ["ハムスター"],
    },
    "魚介料理": {
        "keywords": [
            "シーフード",
            "魚介",
            "サーモン",
            "鮭",
            "サケ",
            "エビ",
            "海老",
            "シュリンプ",
            "プラウン",
            "魚",
            "フィッシュ",
            "白身魚",
            "ホタテ",
            "帆立",
            "イカ",
            "烏賊",
            "イカ墨",
            "タコ",
            "蛸",
            "オクトパス",
            "ツナ",
            "まぐろ",
            "マグロ",
            "カキ",
            "牡蠣",
            "オイスター",
            "蟹",
            "カニ",
            "クラブ",
            "アサリ",
            "ムール貝",
        ],
        "exclude_keywords": [],
    },
    "ピザ": {
        "keywords": [
            "ピザ",
            "ピッツァ",
            "ピッツア",
        ],
        "exclude_keywords": [],
    },
    "スープ": {
        "keywords": [
            "スープ",
            "ヌードルスープ",
            "シチュー",
            "ポタージュ",
            "チャウダー",
            "クラムチャウダー",
            "コンソメ",
            "ブイヨン",
        ],
        "exclude_keywords": [],
    },
    "サラダ": {
        "keywords": [
            "サラダ",
        ],
        "exclude_keywords": [],
    },
    "揚げ物": {
        "keywords": [
            "フライ",
            "フライドポテト",
            "フライドチキン",
            "天ぷら",
            "天麩羅",
            "てんぷら",
            "唐揚げ",
            "から揚げ",
            "からあげ",
            "カツ",
            "とんかつ",
            "ポークカツ",
            "チキンカツ",
            "コロッケ",
            "フリッター",
            "フリット",
        ],
        "exclude_keywords": ["フライパン"],
    },
    "点心": {
        "keywords": [
            "餃子",
            "ギョウザ",
            "ぎょうざ",
            "焼売",
            "シュウマイ",
            "しゅうまい",
            "春巻き",
            "春巻",
            "はるまき",
            "肉まん",
            "あんまん",
            "肉饅頭",
            "小籠包",
            "ショウロンポウ",
            "点心",
            "飲茶",
        ],
        "exclude_keywords": [],
    },
    "デザート": {
        "keywords": [
            "ケーキ",
            "ショートケーキ",
            "チーズケーキ",
            "チョコレートケーキ",
            "タルト",
            "パイ",
            "プリン",
            "カスタードプリン",
            "ゼリー",
            "ジュレ",
            "ムース",
            "ババロア",
            "パンナコッタ",
            "ティラミス",
            "パフェ",
            "サンデー",
            "クレープ",
            "ワッフル",
            "パンケーキ",
            "ホットケーキ",
            "シュークリーム",
            "エクレア",
            "マカロン",
            "クッキー",
            "ビスケット",
            "ブラウニー",
            "マフィン",
        ],
        "exclude_keywords": [
            "アイス",
            "ビーフパイ",
            "ミートパイ",
            "チキンパイ",
            "ポークパイ",
        ],  # アイスは別カテゴリ、肉パイはメインディッシュ
    },
    "アイスクリーム": {
        "keywords": [
            "アイスクリーム",
            "アイス",
            "ジェラート",
            "ソフトクリーム",
            "ソフト",
            "シャーベット",
            "ソルベ",
            "フローズンヨーグルト",
        ],
        "exclude_keywords": ["アイスコーヒー", "アイスティー", "アイスウーロン茶", "アイスミルク"],
    },
}


def load_menus(file_path: str) -> List[Dict]:
    """メニューデータを読み込む"""
    with open(file_path, "r", encoding="utf-8") as f:
        return json.load(f)


def save_menus(file_path: str, menus: List[Dict]):
    """メニューデータを保存する"""
    with open(file_path, "w", encoding="utf-8") as f:
        json.dump(menus, f, ensure_ascii=False, indent=2)


def detect_food_types(menu: Dict, signatures: Dict) -> Set[str]:
    """メニューから料理種類を検出する"""
    name = menu.get("name", "")
    description = menu.get("description", "")
    combined_text = f"{name} {description}"

    detected_types = set()

    for food_type, rules in signatures.items():
        keywords = rules["keywords"]
        exclude_keywords = rules.get("exclude_keywords", [])

        # 除外キーワードチェック
        if any(exclude in combined_text for exclude in exclude_keywords):
            continue

        # キーワードマッチ
        if any(keyword in combined_text for keyword in keywords):
            detected_types.add(food_type)

    return detected_types


def categorize_menus(menus: List[Dict], signatures: Dict, dry_run: bool = False) -> Dict:
    """全メニューに料理種類タグを付与"""
    stats = {
        "total": len(menus),
        "updated": 0,
        "added_tags": 0,
        "by_type": {},
    }

    for menu in menus:
        detected_types = detect_food_types(menu, signatures)

        if not detected_types:
            continue

        # 既存のタグを取得
        existing_tags = set(menu.get("tags", []))

        # 新しいタグを追加
        new_tags = detected_types - existing_tags

        if new_tags:
            if not dry_run:
                menu["tags"] = sorted(existing_tags | detected_types)

            stats["updated"] += 1
            stats["added_tags"] += len(new_tags)

            for tag in new_tags:
                stats["by_type"][tag] = stats["by_type"].get(tag, 0) + 1

    return stats


def main():
    parser = argparse.ArgumentParser(description="メニューに料理種類タグを自動付与")
    parser.add_argument("--dry-run", action="store_true", help="変更をプレビューのみ（実際には保存しない）")
    parser.add_argument("--input", default="data/menus.json", help="入力ファイルパス")
    parser.add_argument("--output", default=None, help="出力ファイルパス（指定しない場合は入力ファイルを上書き）")
    args = parser.parse_args()

    input_path = args.input
    output_path = args.output or input_path

    print(f"📂 メニューデータを読み込み中: {input_path}")
    menus = load_menus(input_path)

    print(f"🔍 料理種類を検出中...")
    print(f"   対象メニュー数: {len(menus)}")
    print(f"   料理種類カテゴリ数: {len(FOOD_TYPE_SIGNATURES)}")

    stats = categorize_menus(menus, FOOD_TYPE_SIGNATURES, dry_run=args.dry_run)

    print("\n" + "=" * 60)
    print("📊 処理結果")
    print("=" * 60)
    print(f"総メニュー数: {stats['total']}")
    print(f"タグ追加されたメニュー数: {stats['updated']}")
    print(f"追加されたタグ総数: {stats['added_tags']}")

    if stats["by_type"]:
        print("\n【料理種類別の追加数】")
        for food_type, count in sorted(stats["by_type"].items(), key=lambda x: x[1], reverse=True):
            print(f"  {food_type}: {count}件")

    if not args.dry_run:
        print(f"\n💾 メニューデータを保存中: {output_path}")
        save_menus(output_path, menus)
        print("✅ 完了！")
    else:
        print("\n⚠️  ドライランモード: ファイルは更新されていません")
        print("   実際に保存するには --dry-run オプションを外して実行してください")


if __name__ == "__main__":
    main()
